...
export class BlogDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    console.log('monting blog details ! ')
    let blogId = this.props.navigation.getParam('blogId', 'NO BLOG');
    return fetch(
      `https://public-api.wordpress.com/rest/v1.1/sites/codinggoodstuff.wordpress.com/posts/${blogId}`
    )
  ...
  render() {
    const blogTagStyles = { img: { display: '' } };
    ...
    return (
      <View style={{ paddingTop: 30 }}>
        {this.state.postLoaded && (
          <ScrollView>
            <Image style={{ width: '100%', height: 200 }} source={{ uri: this.state.postImage }} />
            <HTML
              html={postDetails}
